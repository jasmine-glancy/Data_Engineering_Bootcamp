...
"""
This job deduplicates the records for a user's daily activity, 
then merges today's and yesterday's data to update the activity list
"""
...
def do_host_activity_datelist_transformation(spark, dataframe):
    """Registers the input DataFrame as a temporary view of 'events'
    and runs the SCD transformation SQL. Returns a DataFrame with the 
    updated information on the cumulated hosts"""
...
def main():
    """Loads the 'events' table, applies the SCD transformation, 
    and overwrites the 'hosts_cumulated' table"""
    spark = SparkSession.builder \
        .master("local") \
            .appName("hosts_cumulated") \
                .getOrCreate()
                
    # Ensure 'events' table exists before running transformation
    output_df = do_host_activity_datelist_transformation(spark, spark.table("events"))
